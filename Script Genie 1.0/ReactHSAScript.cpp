...
ReactHSAScript::ReactHSAScript(int x, int y, int width, int height, char* title)
	:ScriptWindow(x, y, width, height, title)
{
	begin();
	//handle file process
	input_stream.open("reactivate_hsa_account_temp.txt");
...
ReactHSAScript::~ReactHSAScript()
{
}


void ReactHSAScript::cb_generate()
{

	string line;
	string lines = "";

	script_name = "wwsa_reactivate_hsa_account_PSQ";
	...
	{
		if (line == "SPOOL wwsa_reactivate_hsa_account_")
		{
			line += "PSQ";
